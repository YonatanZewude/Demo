...
import { Image, Trash2, Upload, Eye, EyeOff } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="mb-6">
          ...
          <p className="mt-2 text-xs text-ink-900/62">Max 5MB. JPG, PNG eller WebP.</p>
        </div>
        ...
        {imagesQuery.data && imagesQuery.data.length > 0 && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {imagesQuery.data.map((image) => (
              <div key={image.id} className="group relative overflow-hidden rounded-2xl">
                ...
                {!image.is_active && (
                  <div className="absolute right-2 top-2 rounded-full bg-black/60 px-2 py-1 text-xs font-semibold text-white">
                    Dold
                  </div>
                )}
              </div>
